...
│   ├── user.interface.ts         # User interface only
│   ├── user.types.ts             # User types and enums
...
```ts
// Interfaces → No prefix, PascalCase — stored in *.interface.ts
interface User { ... }
interface Product { ... }
interface OrderWithItems { ... }

// Types → PascalCase, stored in *.types.ts
type Response<T> = { data: T | null; error: string | null }
type Status = 'pending' | 'active' | 'cancelled'
type PageProps<P = {}, S = {}> = { params: P; searchParams: S }

// Enums → PascalCase, stored in *.types.ts or *.enums.ts
enum UserRole {
  ADMIN = 'ADMIN',
  MEMBER = 'MEMBER',
  VIEWER = 'VIEWER',
...
- Schema names must be PascalCase and ending with `Schema` suffix
...
- Always return `Response<T>` — never throw raw errors to the client
...
- Props must always be typed with `interface ...Props`
...
export class EmailService {
    static buildWelcomeTemplate(name: string): {
        subject: string;
        html: string;
    } {
        return {
...
- [ ] All interfaces have no prefix (not `IUser`, just `User`)
- [ ] All types are PascalCase (not `TUser`, just `User`)
- [ ] All enums are PascalCase (not `EUserRole`, just `UserRole`)
- [ ] All constants in `UPPER_SNAKE_CASE`
- [ ] Zod schemas in `src/schemas/` with `Schema` suffix (PascalCase)
- [ ] Types inferred from schemas — no duplicated type definitions
- [ ] Server Actions return `Response<T>` and check auth first
...
- [ ] Files named descriptively: `user.interface.ts`, `user.types.ts`, `user.actions.ts`, `user.service.ts`
