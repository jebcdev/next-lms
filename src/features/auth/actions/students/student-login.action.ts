...
export const studentLoginAction = async (
    currentUser: StudentLoginData,
): Promise<IGeneralResponse<User>> => {
    try {
        const validatedData =
            StudentLoginSchema.safeParse(currentUser);
        ...
        if (validatedData.data.role !== Role.STUDENT)
            return {
                success: false,
                error: true,
                message:
                    "No tienes permiso para acceder como estudiante",
            };
